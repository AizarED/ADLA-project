...
def movie_corpus_tokeniser(corpus):
    ### IMPLEMENT
    #pass
    tokens=[x.split(" +++$+++ ")[-1].translate(str.maketrans('','',string.punctuation)).split() for x in corpus]
    return tokens
...
text_file = 'movie_lines.txt'                       ### IMPLEMENT (we could write a helper function to format the movie dset like the simple one)
text_file = 'small_corpus.txt'
text_file2='movie_lines_test.txt'  
tokeniser2=movie_corpus_tokeniser

dataset = WordWindowDataset(text_file2, tokeniser2)          # instantiate dataset
