...
def movie_corpus_tokeniser(corpus):
    ### IMPLEMENT
    #pass
    tokens=[(x.split(" +++$+++ ")[-1].translate(str.maketrans('','',string.punctuation)).lower()+" <EOL>").split() for i,x in enumerate(corpus,1) if i<13]
    return tokens
...
with open(text_file, 'rt', encodings='latin1') as f:                         # open the file whilst you read it's contents
...
def clean_words(lines_splitlines):
   dic = { "she's":"she is",  "i'm": "i am", "you’re": "you are",  "you’d": "you would",
   "they're":"they are", 
   "you're":"you are", "don't":"do not", "i've":"i have", "she'd": "she would", 
   "workin'": "working", "goin' ": "going", "where've": "where have", "wouldve":" would have"}
   newline=[]
...
class WordWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ######attempt to pick random pairs with forward probabilities Aizar
